...
#include "PnInput.h"
#include "PnCtrl.h"
#include "../chart dll/Common/SavedHead.h"
#include "../../h/menudef.h"
...
LRESULT CMainWnd::Oub(WPARAM wParam, LPARAM lParam)
{
	...
	tmp1 = CString((char*)lParam,HIWORD(wParam));
			
			if (f.Open("7001.log", CFile::modeCreate|CFile::modeReadWrite))
			{
				f.Write(tmp1,tmp1.GetLength());
				f.Flush();
			}
	...
	CString tmp = CString((char*)lParam,HIWORD(wParam) - SZ_ZCOMHEAD);
...
void CMainWnd::SendRequest()
{
	...
	OutputDebugString(inputStr);
	OutputDebugString("\n===================HEADER END=========================\n");

	OutputDebugString("=====================BODY DATA=======================\n");
	OutputDebugString(gDataH);
	OutputDebugString("\n===================BODY END=========================\n");
