...
BOOL Cdepth::Create(CWnd* parent, void* ptr)
{
	...
	if (CWnd::CreateEx(NULL, m_class, NULL, WS_CHILD|WS_VISIBLE|WS_CLIPSIBLINGS, m_rect, m_parent, idDEPTH, NULL))
	{
		...
		m_opcal.CreateOpSensCal(m_parent, root);

		m_opcal.SetUserName(user);
		m_opcal.ReadCfg();
...
long Cdepth::OnMessage(WPARAM wParam, LPARAM lParam)
{
	...
	case DLL_OUB:	// NOT USER. Use DLL_OUBx Only !!
		m_sync.Lock();
...
// updateX_20060309
	...
	if (m_pBitmapC == NULL) m_pBitmapC = getBitmap(image + _T("호가_설정.bmp"));
	if (m_pBitmapP == NULL) m_pBitmapP = getBitmap(image + _T("호가_시간.bmp"));
...
void Cdepth::dispatch(char* datB, int datL)
{
	...
	if (datL == sz_hogax + sz_Extra)
	{
		dispatchFIVE(datB, datL);
		EW_Dispatch(datB, false);

		if (m_showOHLC == infoSHGA && m_type == ctOption)
			SendMingam();
	}
	else
	{
		...
		parseString(datB, datB+datL, 0x09, fds);
...
void Cdepth::SendMingam()
{
	CString	dats;
	
	dats.Format("20301%c%s\t20021\t20023\t20306\t20034\t", 0x7f, "001");
	SendTR("pooppoop", dats, US_OOP, 2);
	
	dats.Format("20301%c%s\t20021\t20023\t20306\t20034\t", 0x7f, "101");
	SendTR("pooppoop", dats, US_OOP, 2);
}

void Cdepth::parseString(const StringProxy &sp, CHAR del, vector<StringProxy> &retval)
{
	parseString(sp.first, sp.second, del, retval);
}

void Cdepth::ParseOption(vector<StringProxy> fds)
{
	if (fds.size() != 8)
		return;
	...
	m_cdkr = Str2Double(fds[0]);
	sprintf(buff, "%.2f", m_cdkr);
	m_opcal.SetCd(buff);

	m_bjis = Str2Double(fds[1])/1000000.0;
	sprintf(buff, "%.6f", m_bjis);
	m_opcal.SetBedangJisu(buff);

	m_clday = Str2Double(fds[2]);
	sprintf(buff, "%.2f", m_clday);
	m_opcal.SetCalRemainDays(buff);

	m_bzday = Str2Double(fds[3]);
	sprintf(buff, "%.2f", m_bzday);
	m_opcal.SetBzRemainDays(buff);

	GenOptionData();
	SetTimer(1000,20000,NULL);
}

void Cdepth::GenOptionData()
{
	char	buff[64]{};
	const	double strk = GetOptionStrike(m_code);
	
	m_opcal.SetJCode(m_code);
	sprintf(buff, "%.2f", atof(m_items.GetAt(infoCURR)->m_data));
	m_opcal.SetCurrValue(buff);
	
	sprintf(buff, "%.1f", strk);
	m_opcal.SetStrike(buff);
	
	m_opcal.Calc(0);
	
	m_items.GetAt(infoIRGA)->m_data = format(m_opcal.m_IOpSensCal->GetTheoryValue(), infoIRGA);
	m_items.GetAt(infoNJBD)->m_data = format(m_opcal.m_IOpSensCal->GetIv(), infoNJBD);
	m_items.GetAt(infoDELT)->m_data = format(m_opcal.m_IOpSensCal->GetDelta(), infoDELT);
	m_items.GetAt(infoGAMM)->m_data = format(m_opcal.m_IOpSensCal->GetGamma(), infoGAMM);
	
	Invalidate();
}

void Cdepth::ParseJISU(vector<StringProxy> fds)
{
// 	vector<StringProxy> fds;
// 	parseString(dat, dat+len, 0x09, fds);
	...
	CString	dats;
	CString strCode = (LPCSTR)m_parent->SendMessage(WM_USER, MAKEWPARAM(codeDLL, MAKEWORD(getCODEfuture, 2)), 0);
	
	dats.Format("30301%c%s\t30021\t30022\t31111\t31023\t31024\t31204\t31027\t31201\t31206\t31033\t31203\t31205\t31202\t", 0x7f, strCode);
	SendTR("pooppoop", dats, US_OOP, 2);

	//GenOptionData();
}

void Cdepth::ParseFUTURE(vector<StringProxy> fds)
{
	if (fds.size() != 13)
		return;
	...
	CString	fjcode = (LPCSTR)m_parent->SendMessage(WM_USER, MAKEWPARAM(codeDLL, MAKEWORD(getCODEfuture, 2)), 0);
	if (fjcode == code)
	{
		char	buff[16]{};

		sprintf(buff, "%.2f", fabs(Str2Double(fds[3])));
		m_opcal.SetFutureCur(buff);
	}

	CString	dats;
	dats.Format("40301%c%s\t40332\t40323\t40314\t40370\t40361\t40369\t40368\t40367\t", 0x7f, m_code);
	SendTR("pooppoop", dats, US_OOP,4);

	//GenOptionData();
}

double Cdepth::GetOptionStrike(CString code)
{
	double	strike = 0;
	
	strike = atof(code.Mid(5, 3));
	if (code[7] == '2' || code[7] == '7')
		strike = strike + 0.5;
...
{
	CString	tmps;
	struct	_hogax*	hoga = (struct _hogax *) datB;
...
// updateX_20060309
	...
	tmps = CString(hoga->gjga, sizeof(hoga->gjga)); tmps.Trim();
...
void Cdepth::alert(struct _alertR* alertR)
{
	...
	for (POSITION pos = m_indices.GetStartPosition(); pos; )
	{
		m_indices.GetNextAssoc(pos, key, index);
		...
		if (index == infoIRGA || index == infoNJBD || index == infoDELT || index == infoGAMM)
			continue;
...
void Cdepth::drawBong(CDC* dc)
{
	...
	CPen*	pen = (siga < m_curr) ? getPen(m_clrUp, 1) : getPen(m_clrDown, 1);
...
void Cdepth::drawBongMGVL(CDC* dc)
{
	...
	CPen*	pen = (siga < jjga) ? getPen(m_clrUp, 1) : getPen(m_clrDown, 1);
	CPen*	pPen = dc->SelectObject(pen);
...
void Cdepth::drawInfo(CDC* dc)
{
	...
	if (m_showOHLC == infoMGVL)
	{
		...
		{
			dc->SetTextColor(m_clrDataFg);
			rect.top = offset + (int)(m_rowH * ii);
			rect.bottom = offset + (int)(m_rowH * (ii+1));
	...
	else if (m_showOHLC == infoSHGA && m_type == ctFuture)
	{
		...
		for (int ii = 0; ii < 5; ii++)
		{
			...
			case 3:
				...
				//2015.09.01 KSJ Basis -일때 추가해주도록 변경
				if (m_items.GetAt(infoBASE)->m_fRGB == m_clrDown)
				{
			...
			case 4:
				...
				//2015.09.01 KSJ 괴리율 -일때 추가해주도록 변경
				if (m_items.GetAt(infoRATE)->m_fRGB == m_clrDown)
				{
...
void Cdepth::calculateRowHeight()
{
	...
	const	int	height = m_rect.Height() - 1;
...
void Cdepth::setFocus(CPoint point)
{
	 ...
	 if (m_bBong)
	{
		...
		rc = m_rcInfo;			//2015.11.10 KSJ 위치지정
		rc.top = rc.top + rc.Height() / 2;
		if (rc.PtInRect(point))
		{
			strTooltip = GetBongMGVLStr();
...
CString Cdepth::format(CString data, int index)
{
	...
	CString firstword, secondword;
	firstword = data.Mid(0,1);
	secondword = data.Mid(1,1);
	
	if (firstword.Compare(" ") == 0 && (secondword.Compare("+") == 0 || secondword.Compare("-") == 0))
		data.TrimRight();
	else
		data.Trim();
...
void Cdepth::SendTR(CString name, CString data, BYTE type, int key)
{
	...
	m_parent->SendMessage(WM_USER, MAKEWPARAM(invokeTRx, datl), (LPARAM)datb.get());
	datb.reset();
}
...
{
	CString str;
	CString siga, koga, jega;
	CString jgga, ma5, ma10;

	std::shared_ptr<Citem> pItem = m_items.GetAt(openPrice);
	siga = pItem->m_data;
	pItem = m_items.GetAt(highPrice);
	koga = pItem->m_data;
	pItem = m_items.GetAt(lowPrice);
	jega = pItem->m_data;
	jgga.Format("%.2f", m_curr);
	
	ma5.Format("%.2f", (m_sum4 + m_curr) / 5);
	ma10.Format("%.2f", (m_sum9 + m_curr) / 10);
...
//2015.11.10 KSJ 데이터 수정
CString Cdepth::GetBongMGVLStr()
{
	CString str;
	CString	gijun, mgjv, mgdf;
	CString siga, sigabi;
	CString koga, kogabi;
	CString jega, jegabi;

	std::shared_ptr<Citem> pItem = m_items.GetAt(gijunPrice);
	gijun = pItem->m_data;

	pItem = m_items.GetAt(infoMGJV);
	mgjv = pItem->m_data;
	pItem = m_items.GetAt(infoMGDF);
	mgdf = pItem->m_data;

	pItem = m_items.GetAt(openPrice);
	siga = pItem->m_data;
	sigabi.Format("%.2f", atof(siga) - atof(gijun));

	pItem = m_items.GetAt(highPrice);
	koga = pItem->m_data;
	kogabi.Format("%.2f", atof(koga) - atof(gijun));

	pItem = m_items.GetAt(lowPrice);
	jega = pItem->m_data;
	jegabi.Format("%.2f", atof(jega) - atof(gijun));

//	str = _T("미결제약정\t\n시가\t") + siga + _T("\n고가\t") + koga + _T("\n저가\t") + jega + _T("\n종가\t") + jgga + _T("\n");
	str = _T("미결제(증감)\t") + mgjv + _T("(") + mgdf + _T(")\n시가(대비)\t") + siga + _T("(") + sigabi + _T(")\n고가(대비)\t") + koga + _T("(") + kogabi + _T(")\n저가(대비)\t") + jega + _T("(") + jegabi + _T(")\n");
...
void Cdepth::drawButton(CDC *pDC)
{
	if (m_bConfig)
	{
		...
		{
			drawButton(pDC, m_rcConfig, m_pBitmapC);
		}
		if (!m_rcConfigL.IsRectEmpty())
		{
			drawButton(pDC, m_rcConfigL, m_pBitmapP);
		}
		if (!m_rcConfigR.IsRectEmpty())
		{
			drawButton(pDC, m_rcConfigR, m_pBitmapP);
		}
	}
}
...
{
	CMenu	cMenu;
	const	char*	const	pDisplay[4] = { "표시 안함", "정보1 표시", "정보2 표시", "정보3 표시" };
...
void Cdepth::InitEnv()
{
	...
	m_percent = pcNo;	if (m_config.hdyul) m_percent	= pcJgga;//pcGijun;	//2014.06.09 KSJ 종가 --> 기준가 보류
...
void Cdepth::EW_Dispatch(char *dataB, bool bTEN)
{
	...
	if (bTEN)
	{
		...
		if (m_pContract)
			m_pContract->Dispatch(&dataB[sz_hoga], digit);
		if (m_pMBong)
			m_pMBong->Dispatch(&dataB[sz_hoga], CString(hoga->shga, sizeof(hoga->shga)), CString(hoga->hhga, sizeof(hoga->hhga)), digit);
	}
	else
	{
		...
		if (m_pContract)
			m_pContract->Dispatch(&dataB[sz_hogax], digit);
		if (m_pMBong)
			m_pMBong->Dispatch(&dataB[sz_hogax], CString(hoga->shga, sizeof(hoga->shga)), CString(hoga->hhga, sizeof(hoga->hhga)), digit);
	}
}
...
void Cdepth::OnTimer(UINT nIDEvent) 
{
	//KillTimer(1000);
