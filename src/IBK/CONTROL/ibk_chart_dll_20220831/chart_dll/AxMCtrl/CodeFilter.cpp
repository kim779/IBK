...
BOOL FP_KospiKRX(LPVOID node, int kind, int subkind, int index)
{
	return ((struct hjcode*)node)->unio == 1;
}
...
/*
BOOL FP_Kospi200Manufacture(LPVOID node, int kind, int subkind, int index)
{
	return ((struct hjcode*)node)->kosd == jmKOSPI && ((struct hjcode*)node)->k200 == K200_MANUFACTURE;
}

BOOL FP_Kospi200Electronic(LPVOID node, int kind, int subkind, int index)
{
	return ((struct hjcode*)node)->kosd == jmKOSPI && ((struct hjcode*)node)->k200 == K200_ELECTRONIC;
}

BOOL FP_Kospi200Building(LPVOID node, int kind, int subkind, int index)
{
	return ((struct hjcode*)node)->kosd == jmKOSPI && ((struct hjcode*)node)->k200 == K200_BUILDING;
}

BOOL FP_Kospi200Distribution(LPVOID node, int kind, int subkind, int index)
{
	return ((struct hjcode*)node)->kosd == jmKOSPI && ((struct hjcode*)node)->k200 == K200_DISTRIBUTION;
}

BOOL FP_Kospi200Finance(LPVOID node, int kind, int subkind, int index)
{
	return ((struct hjcode*)node)->kosd == jmKOSPI && ((struct hjcode*)node)->k200 == K200_FINANCE;
}
*/
...
/*			case ID_KOSPI200_MANUFACTURE:	return FP_Kospi200Manufacture;
			case ID_KOSPI200_ELECTRONIC:	return FP_Kospi200Electronic;
			case ID_KOSPI200_BUILDING:		return FP_Kospi200Building;
			case ID_KOSPI200_DISTRIBUTION:	return FP_Kospi200Distribution;
			case ID_KOSPI200_FINANCE:		return FP_Kospi200Finance;
*/
			case 99:			return FP_KospiKRX;
		}
	}
	return NULL;
}
...
BOOL FP_KosdaqManufacture(LPVOID node, int kind, int subkind, int index)
{
	return ((struct hjcode*)node)->kosd == jmKOSDAQ && ((struct hjcode*)node)->size == 4 &&
		((struct hjcode*)node)->jjug == 2;
}
...
CODEFILTERPROC GetKosdaqFilterProc(int subkind, int index)
{
	...
	else if (index == 5)		return FP_KosdaqManufacture;
	else if (index <= 14 || index == 41)		return FP_KosdaqMiddle;
...
BOOL FP_ELW_Asset(LPVOID node, int kind, int subkind, int index)
{
	struct elwcode* p = (struct elwcode*)node;
	...
	for (int i = 0; i < 5; i++) {
		s = CString(p->gcjs[i], ARRAY_SIZE(p->gcjs[i]));
		s.TrimRight();

		if (index == ID_KOSPI200 && s == "KOSPI200")
			return TRUE;
		else if (s.GetLength() > 1 && atoi(s.Mid(1)) == index)
			return TRUE;
	}
...
BOOL FP_ELW_List(LPVOID node, int kind, int subkind, int index)
{
	struct elwcode* p = (struct elwcode*)node;
