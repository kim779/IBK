	...
	case VK_RETURN:
		...
		case CMB_GIGAN:
			if (!tmpS.IsEmpty())
			{
				...
				if (gigan > MAX_DATA_COUNT)
				{
					CString	str;
					str.Format("%d", MAX_DATA_COUNT);
					SetWindowText(str);
					m_pParent->SendMessage(GEV_INPUT, MAKEWPARAM(INPUT_CHANGE, IP_GIGAN), MAX_DATA_COUNT);
				}
				else
				{	
					m_pParent->SendMessage(GEV_INPUT, MAKEWPARAM(INPUT_CHANGE, IP_GIGAN), gigan);
				}
			}
			break;
		default:
