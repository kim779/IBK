...
: CPnBaseWnd(pView, pParent, pFont, ctrlID, info)
{
	...
	m_bGrid = false;//(envinfo->display.dwDspOption & DO_VIEWGRID)? true:false;
	m_bSiseOnly = FALSE;
}
...
*/			break;
	...
	case CTRL_ENV:
		...
		case ctrlDisplay:
			...
			if (m_iMainKind >= GK_PV)
				m_iMainKind = m_pwndGView->SendMessage(GEV_VIEW, MAKEWPARAM(viewMulti, mainInput), 0);
			UpdateScroll();
			break;
		case ctrlEnable:
			if (m_iAutoScroll)
...
int CPnCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	path = m_pApp->GetImagePath(m_pwndView, CTRL2IMAGE);
	m_pimglstCtrl2 = m_pApp->GetImage(imgCtrl2, path);

	m_bMouseIN = false;

	m_bSiseOnly = m_pwndGView->SendMessage(GEV_VIEW, MAKEWPARAM(viewMain, isSiseOnly), 0);
...
void CPnCtrl::OnMouseLeave(UINT nFlags, CPoint point)
{
	if (m_bLButton)
		lbuttonup(point);
	else if (m_iMoveID)
	{
		struct _comp* ocomp = SearchCom(m_iMoveID);
		if (ocomp && !(m_iMoveID == IC_GRID && m_bGrid))
			InvalidateCom(ocomp, btnUp);
	}
	m_iMoveID = 0;
	m_bMouseIN = FALSE;
...
void CPnCtrl::ArrangeCtrl(CRect wRC)
{
	...
	int	btnWidth = szCtrlImgX*btnCnt - 2;
	
	wRC.DeflateRect(1, 0);
	int	resizeWidth = wRC.Width() - btnWidth;
	int	basicWidth = sliderWIDTH;
...
void CPnCtrl::drawButton(CDC* pDC, struct _comp* comp)
{
	...
	case IC_SCROLL:	case IC_SLIDER:
		break;
	case IC_LINK1:	case IC_LINK2:	case IC_LINK3:	case IC_LINK4:
		DrawButton(pDC, dtButton, comp, m_pimglstCtrl2);
		break;
	default:
...
void CPnCtrl::ReadDisplay()
{
/*
	if (!m_pDisplay)	return;

	struct _envInfo	*envinfo = (_envInfo*)m_pwndGView->SendMessage(GEV_CHART, MAKEWPARAM(CHART_DATA, REQUEST_CUR_ENV), 0);
	int	unit = envinfo->datainfo.btUnit;
	int	index = envinfo->datainfo.btIndex;
	...
	int disCnt = envinfo->datainfo.aaDayInfo[uIndex][index-GI_DAY].dwDisCnt;

	CString	tmpS;
	tmpS.Format("%d", disCnt);
	m_pDisplay->SetWindowText(tmpS);*/
}
