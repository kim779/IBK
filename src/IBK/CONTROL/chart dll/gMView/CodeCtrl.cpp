...
CCodeCombo::CCodeCombo(class CCodeCtrl *pParent)
{
	m_pParent = pParent;
...
bool CCodeCombo::JCodeLoad(CString tabPath)
{
	...
	{
		AfxGetMainWnd()->MessageBox("종목 화일을 읽을 수 없습니다.", "확인", MB_OK);
		CloseHandle(fh);
		return false;
	}
...
int CCodeCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_pCombo->Create(WS_CHILD|WS_VSCROLL|CBS_DROPDOWN, CRect(0, 0, 10, 10), this, 0);
