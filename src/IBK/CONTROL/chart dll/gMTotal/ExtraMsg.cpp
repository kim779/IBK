	...
	case SISE_TR_DATA:
		{
			...
			if (m_pMainWnd->m_bForeign)
				pTD->iSiseLen[0] =  receiveFORData(pTD->pcData[0], pTD->iLen[0]);
			else
...
int CExtraMsg::ReceiveData(char* pcData, int iLen)
{
	...
	m_pMainWnd->m_strJName.TrimRight(" ");
	m_pMainWnd->m_strJName.Replace("&", "&&");
	if (m_pMainWnd->m_strCode == "001")
		m_pMainWnd->m_strJName = "(거)" + m_pMainWnd->m_strJName;
	else if (m_pMainWnd->m_strCode == "201")
		m_pMainWnd->m_strJName = "(코)" + m_pMainWnd->m_strJName;
