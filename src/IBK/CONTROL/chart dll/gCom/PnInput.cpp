...
*/
		...
		case getCalendar:
			{
				...
				sprintf((char*)lParam, "%04d%02d%02d", time.GetYear(), time.GetMonth(), time.GetDay());
			}
			break;
		case getCurGapList:
			{
		...
		case setCodeInfo:
			switch(m_iDtUnit)
			{
			case GU_FOREIGN: m_pCodeCB->AddHistory(STRG,  m_pCodeCB->GetEditData(), GetSiseName()); break;
			case GU_INDEX:   m_pCodeCB->AddHistory(ITRG,  m_pCodeCB->GetEditData(), GetSiseName()); break;
			case GU_COFUT:   m_pCodeCB->AddHistory(CFTRG, m_pCodeCB->GetEditData(), GetSiseName()); break;
			}
			...
			if (strCode != K200_INDEX && strCode != K200_FUTURE && strCode != STAR_INDEX && strCode != STAR_FUTURE)
			{
				...
				case foFmFut:	// 20090918 : ADD : 현물 선물 코드 처리
					if (m_bStar)
					{
						...
						if (comp)
						{
							sprintf(comp->str, K200_NAME);
							sprintf(comp->comment, K200_TIP);
							InvalidateCom(comp, comp->status);
						}
					}
					break;
				case foStar:
					if (!m_bStar)
					{
						...
						if (comp)
						{
							sprintf(comp->str, STAR_NAME);
							sprintf(comp->comment, STAR_TIP);
							InvalidateCom(comp, comp->status);
						}
					}
					break;
...
int CPnInput::GetTRInfo(char* pcHeader)
{
	...
	sprintf(pcHeader, "%s%c%s\t%s", strCode , 0x7F, strTemp, m_strOutSymbol);
...
int CPnInput::GetTRInfo2(char *pHeader)
{
	...
	sprintf(pHeader, "%s%c%s\t", codeS, 0x7F, m_strValidCode);
...
/*	else if (m_iDtUnit == GU_INDEX)
	...
	if (m_iDtUnit == GU_COFUT)
	{
		//m_pCodeCB->AddHistory(htySymbol, code, NULL);	
	}
...
const char *units[] = {"주식", "업종", "선옵", "해외", "상품", "주선"};
...
int CPnInput::WhichFOCode(LPCTSTR code)
{
	...
	if (cd != K200_INDEX && cd != K200_FUTURE && cd != STAR_INDEX && cd != STAR_FUTURE)
	{
		...
		else if (cd[0] == '1' || cd[0] == '4')	// 1xxxxxxx
		{
