...
public:
	...
	void    SetListboxPos(int x, int y);
	BOOL    ListboxLButtonDown(int x, int y);
	BOOL    ListboxLButtonUp(int x, int y);
	CString GetHistory(LPCSTR del1, LPCSTR del2);
...
private:
	...
	CMap<CString,LPCSTR,CString,LPCSTR> m_SFCode;
...
public:
	...
	void	LButtonUp();
	void	Draw(CDC *pDC);
	CString	GetPath(int type);

	void    SetHistory(LPCSTR key, LPCSTR history);
	void    DeleteHistory(LPCSTR code);
