...
LRESULT CSubList::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	if (message == WM_LBUTTONUP)
	{
		...
		if (m_pCodeCtrl->m_pCombo->ListboxLButtonUp(LOWORD(lParam), HIWORD(lParam))) 
			return 0;
	}
	else if (message == WM_MOUSEMOVE)
	{
		m_pCodeCtrl->m_pCombo->SetListboxPos(LOWORD(lParam), HIWORD(lParam));
	}
...
CCodeCombo::CCodeCombo(class CCodeCtrl *pParent, int iTextHeight, int TextGap)
{
	...
	CString imgPath;
	imgPath.Format("%s\\%s\\TK_CLOSE.BMP", pParent->GetPath(getRootPath), IMAGEDIR);
	HANDLE handle = ::LoadImage(AfxGetInstanceHandle(), imgPath, IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS|LR_LOADFROMFILE);
	if (handle)
		m_bmpClose.Attach((HBITMAP)handle);
...
void CCodeCombo::OnSelchange() 
{
	...
	if (index >= 0)
	{
		if (IsCodeListMode() && m_pJCode.GetSize()>=index)
		{
		...
		else if (IsHistoryMode() && m_pHCode.GetSize()>=index)
		{
			jCode = m_pHCode.GetAt(index);
			m_pParent->m_pEdit->ListCode(jCode.Code);
			m_pParent->m_pEdit->SetFocus();
			//MessageBox(jCode.Code);
		}
	}
}
...
void CCodeCombo::InitCodeList()
{
	...
	for (int ii = 0; ii < m_pJCode.GetSize(); ii++)
	{
		...
		Str.Format("%s  %s", jCode.Code.GetAt(0) == 'A' ? jCode.Code.Mid(1) : jCode.Code, jCode.Name); // 2006.11.16 lsh (윗줄수정)
		AddString(Str);
	}
...
bool CCodeCombo::UCodeLoad(CString tabPath)
{
	...
	{	
		AfxGetMainWnd()->MessageBox("업종 화일을 읽을 수 없습니다.", COMPANYNAME);
		return FALSE;
	}
...
void CCodeCombo::DrawItem( LPDRAWITEMSTRUCT di )
{
	CString text;
	CDC memDC, *pDC = CDC::FromHandle(di->hDC);
	CRect *pRC = (CRect*)&di->rcItem;
	GetLBText(di->itemID, text);
	
	COLORREF fgCol = (di->itemState&ODS_SELECTED) ? RGB(0xFF,0xFF,0xFF) : RGB(0x00,0x00,0x00);
	COLORREF bgCol = (di->itemState&ODS_SELECTED) ? RGB(0x0A,0x24,0x6A) : RGB(0xFF,0xFF,0xFF);

	pDC->FillSolidRect(pRC, bgCol);
	pRC->DeflateRect(CSize(3,2));
	pDC->SetTextColor(fgCol);
	pDC->DrawText(text, pRC, DT_SINGLELINE|DT_LEFT);
	pRC->DeflateRect(CSize(-3,-2));
	...
	do
	{
		if (di->itemState&ODS_SELECTED)
		{
			if (IsHistoryMode())
			{
				...
				if (!memDC.GetSafeHdc()) break;
				CBitmap *pOldBmp = memDC.SelectObject(&m_bmpClose);

				m_rcClose.SetRect(pRC->right-16, pRC->top+1, pRC->right-1, pRC->bottom-1);
				m_lbCurrIdx = di->itemID;

				if (m_rcClose.PtInRect(m_lbPoint))
					pDC->BitBlt(m_rcClose.left, m_rcClose.top, m_rcClose.Width(), m_rcClose.Height(), &memDC,  0, 0, SRCCOPY);
				else
					pDC->BitBlt(m_rcClose.left, m_rcClose.top, m_rcClose.Width(), m_rcClose.Height(), &memDC, 15, 0, SRCCOPY);
			}
			else
			{
				m_rcClose.SetRect(0,0,0,0);
			}
		}
	} while(FALSE);
}

void CCodeCombo::SetListboxPos( int x, int y )
{
	m_lbPoint.x = x;
	m_lbPoint.y = y;

	CRect rc(x-15,y-15, x+15, y+15);
	::InvalidateRect(m_hList, &rc, FALSE);
}

BOOL CCodeCombo::ListboxLButtonDown( int x, int y )
{
	return m_rcClose.PtInRect(CPoint(x,y));
}


BOOL CCodeCombo::ListboxLButtonUp( int x, int y )
{
	if (m_rcClose.PtInRect(CPoint(x,y)))
	{
		if (GetCount()<=m_lbCurrIdx) return FALSE;

		CString code = m_pHCode.ElementAt(m_lbCurrIdx).Code;

		// 아래 두줄은 순서가 바뀌면 안된다... (지수/해외의 경우 파일로 히스토리를 유지하기때문)
		m_pHCode.RemoveAt(m_lbCurrIdx);
		m_pParent->DeleteHistory(code);
		DeleteString(m_lbCurrIdx);
		m_rcClose.SetRect(0, 0, 0, 0);

		// 이거 안해주면 WM_MOUSEMOVE해도 SEL이 안되는수가 있다!! 
		::PostMessage(m_hList, LB_SETCURSEL, m_lbCurrIdx, 0);
		::PostMessage(m_hList, WM_LBUTTONDOWN, 0, MAKELPARAM(x,y));
...
CString CCodeCombo::GetHistory( LPCSTR del1, LPCSTR del2 )
{
	CString data;
	for(int n=0; n<m_pHCode.GetSize(); ++n)
	{
		data += m_pHCode[n].Code;
		data += del1;
		data += m_pHCode[n].Name;
		data += del2;
	}
	return data;
}
...
bool CCodeCombo::SFCodeLoad( CString tabPath )
{
	...
	if (!file.Open(path, CFile::modeRead|CFile::typeBinary))
	{	
		AfxGetMainWnd()->MessageBox("주식선물 화일을 읽을 수 없습니다.", COMPANYNAME);
		return FALSE;
	}

	struct sfcode sfcode;
	DWORD nFSize = file.GetLength() / sizeof(sfcode);
	
	for (DWORD ii = 0; ii < nFSize; ii++)
	{
		file.Read(&sfcode, sizeof(sfcode));
		
		CString code(sfcode.codx, sizeof(sfcode.codx));
		CString hnam(sfcode.hnam, sizeof(sfcode.hnam));
		m_SFCode.SetAt(code, hnam);
	}
...
//	if (m_Unit == GU_CODE)
	{
		...
		m_pParent->m_pCombo->SendMessage(WM_MOUSEWHEEL, MAKEWPARAM(nFlags, zDelta), MAKELPARAM(pt.x,pt.y));
...
int CCodeCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_pCombo->Create(WS_CHILD|WS_VSCROLL|CBS_DROPDOWN|CBS_OWNERDRAWFIXED|CBS_HASSTRINGS, //|CBS_OWNERDRAWVARIABLE|CBS_HASSTRINGS, // IBK
...
void CCodeCtrl::HistoryMode()
{
	...
	case GU_INDEX:
		...
		break;
	case GU_COFUT:
		history = GetHistory(CFTRG);
		break;
	case GU_FUTURE:	case GU_OPTION:	case GU_COOPT:
		symbol = FOTRG;
		history = (char *)m_pView->SendMessage(GEV_VIEW, MAKEWPARAM(viewMain, getHistory), (long)symbol.operator LPCTSTR());
		break;
...
{
	CString fileName;
	fileName.Format("%s%s_history.dat", m_userPath, key);
	return fileName;
}
...
CString CCodeCtrl::GetPath( int type )
{
	return (char *)m_pView->SendMessage(GEV_VIEW, MAKEWPARAM(viewInterface, type), 0);
}

void CCodeCtrl::DeleteHistory( LPCSTR code )
{
	CString	symbol, htySymbol;
	
	switch (m_Unit)
	{
	case GU_CODE:
		htySymbol.Format("%s\t%s", JTRG, code);
		((CPnBaseWnd*)m_pParent)->Variant(codeCCx, (LPARAM)(LPCSTR)htySymbol);
		break;
	case GU_INDEX:
		SetHistory(ITRG, m_pCombo->GetHistory(" ", "\t"));
		break;
	case GU_COFUT:
		SetHistory(CFTRG, m_pCombo->GetHistory(" ", "\t"));
		break;
	case GU_FUTURE:	case GU_OPTION:	case GU_COOPT:
		htySymbol.Format("%s\t%s", FOTRG, code);
		((CPnBaseWnd*)m_pParent)->Variant(codeCCx, (LPARAM)(LPCSTR)htySymbol);
		break;
	case GU_FFUTURE:	// 20090916 : ADD : 상품 선물
	case GU_CFUTURE:	// 20090916 : ADD : 채권, 금리 선물
		htySymbol.Format("%s\t%s", CDFTRG, code);
		((CPnBaseWnd*)m_pParent)->Variant(codeCCx, (LPARAM)(LPCSTR)htySymbol);
		break;
	case GU_FOREIGN:
		SetHistory(STRG, m_pCombo->GetHistory(" ", "\t"));
		break;
	default:
		return;
	}
...
void CCodeCtrl::SetHistory( LPCSTR key, LPCSTR history )
{
	CString fileName = GetHistoryFileName(key);
	CFile* pFile;
	
	TRY {
		pFile = new CFile(fileName, CFile::modeWrite||CFile::modeCreate);
	} 
	CATCH (CFileException, e) {
		return;
	}
	...
	int dLen = strlen(history);
	pFile->Write(history, dLen);
	pFile->SetLength(dLen);
	pFile->Flush();
	pFile->Close();
	delete pFile;
}
