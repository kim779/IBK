...
static void ShowLastError()
{
	...
	if(error)
	{
		...
		{
			// Display the string.
			AfxGetMainWnd()->MessageBox((LPCTSTR)lpMsgBuf, _T("Error"), MB_OK | MB_ICONINFORMATION );
			// Free the buffer.
			LocalFree( lpMsgBuf );
		}
		else
		{
			...
			// Display the string.
			AfxGetMainWnd()->MessageBox(temp, _T("Error"), MB_OK | MB_ICONINFORMATION );
		}
	}
}
...
static COLORREF GetXpHighlightColor()
{
	if(g_bHighContrast)
...
COLORREF CNewMenu::GetMenuColor()
{
...
#ifdef _TRACE_MENU_
 //   AfxTrace(_T("DrawItem: 0x%lX Menus %ld\n"),lpDIS->itemState,lpDIS->itemData);
#endif
	...
	if(bIsMenuBar && (lpDIS->itemState&ODS_SELECTED) )
	{
		if(!(lpDIS->itemState&ODS_INACTIVE) )
		{
			...
			{ 
				...
				Pen.DeleteObject();
				colorBorder = g_bHighContrast?GetSysColor(COLOR_BTNTEXT):DarkenColor(128,GetMenuBarColor());
	...
	else
	{
		...
		if(!strText.IsEmpty())
		{
			...
			else
			{
				...
				if(	g_bHighContrast && !bIsMenuBar && !(state&ODS_SELECTED) )
...
void CNewMenu::DrawItem_Icy (LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
	...
	COLORREF colorGrayed = g_bHighContrast?GetSysColor(COLOR_BTNSHADOW):pDC->GetNearestColor(DarkenColor(70,color3DShadow)); 
	...
	if(state & MF_SEPARATOR)
	{
		...
		else
		{
			...
			//pDC->FillSolidRect(rect,color3DHilight);
			//pDC->FillSolidRect(rect.left,rect.top,rect.Width(),2,color3DShadow);
		}
	}
	else
	{
		...
		if(bIsMenuBar)
		{
			...
			rect.OffsetRect(-1,1);
			rect2=rect;
			if( g_bHighContrast || (state&ODS_INACTIVE) || (!(state&ODS_HOTLIGHT) && !(state&ODS_SELECTED)) )
			{
				pDC->FillSolidRect(rect,colorBack);
			}
			else
		...
		if(state&ODS_SELECTED)
		{
			...
			{
				pDC->Draw3dRect(rect ,color3DShadow,color3DHilight);
			}
		}
...
BOOL CNewMenu::InsertODMenu(UINT nPosition, LPCTSTR lpstrText, UINT nFlags, UINT nID, int nIconNormal)
{
	int nIndex = -1;
...
BOOL CNewMenu::InsertODMenu(UINT nPosition, LPCTSTR lpstrText, UINT nFlags, UINT nID, CNewMenuIcons* pIcons, int nIndex)
{
	if(!(nFlags & MF_BYPOSITION))
	{
		int iPosition =0;
...
BOOL CNewMenu::SetMenuText(UINT id, CString string, UINT nFlags/*= MF_BYPOSITION*/ )
{
	...
	if(MF_BYPOSITION&nFlags)
	{
		...
		if(id<UINT(numMenuItems))
		{
			...
			pItemData->SetString(string);
			returnflag = CMenu::ModifyMenu(id,MF_BYPOSITION|MF_STRING | nState, pItemData->m_nID, string);
			ModifyMenu(id,MF_BYPOSITION | MF_OWNERDRAW,pItemData->m_nID,(LPCTSTR)pItemData);
		}
	}
	else
	{
...
{ 
	...
	{
		g_bHighContrast = ((highcontrast.dwFlags&HCF_HIGHCONTRASTON)!=0);
	}
