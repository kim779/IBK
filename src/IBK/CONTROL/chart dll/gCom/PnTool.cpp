...
BEGIN_MESSAGE_MAP(CPnTool, CWnd)
	...
	ON_WM_MENUCHAR()
	ON_WM_DESTROY()
	ON_COMMAND_RANGE(ID_TOOL_HLINE, ID_TOOL_TRILINELEVEL, OnMenuCheck)
	//}}AFX_MSG_MAP
...
void CPnTool::OnMouseLeave(UINT nFlags, CPoint point)
{
	m_bMouseIN = FALSE;
...
void CPnTool::DrawTool(CDC *pDC)
{
	...
	for (ii = popLINE; ii <= popLINE + maxPOP; ii++)
	{
...
void CPnTool::ToolConfig()
{
	...
	if (m_pwndGView)
	{
		...
		if (bResult)
		{
			...
			for (pos = m_mapPnToolInfo.GetStartPosition(); pos; )
			{
				CPnToolInfo* pPTI;
