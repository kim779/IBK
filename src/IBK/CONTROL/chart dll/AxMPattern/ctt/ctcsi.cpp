	...
	for (i1 = 0; i1 < 120; i1++)
	{
		...
		if (mrec.dflag == '0')
		{
			if (mrec.csflag == 'C')
			{
				...
				sprintf(symbol, "%2.2s%2.2s%2.2s", mrec.csym, buffer, mrec.dy);
			}
			else
			{
			...
			buffer[20] = '\0';
			l_Rtrim(buffer);
			if (!stricmp(name, symbol) || !stricmp(name, buffer))
			{
						 ...
						 unsigned short *actualdate)
{
	...
	if (path[0] != '\0' && path[strlen(path) - 1] != ':' && path[strlen(path) - 1] != '/')
		...
		sprintf(buffer, "%sF%03.3i.DTA", path, fnum);
	if ((fp1 = fopen(buffer, "rb")) == NULL)
		return (ctErrFileOpen);
...
short WINAPI l_CSIread(ctBars *b1, char *path, char *name, long start, short  cnt, short  allocate)
{
	...
	if (mrec.csflag == 'C')
		sprintf(b1->symbol, "%2.2s%2.2s%2.2s", mrec.csym, mrec.dm, mrec.dy);
	else
		sprintf(b1->symbol, "%6.6s", mrec.ssym);
	l_Rtrim(b1->symbol);
	b1->cusip[0] = '\0';
