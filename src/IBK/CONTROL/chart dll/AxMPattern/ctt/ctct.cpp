				...
				if (stack[ptr] == 0)          /* i.e. first time in this master */
				{
					for (i1 = 0; i1 < numentries; i1++)
					{
						...
						else
						{
							...
							strncpy(buffer, dir.name, 16);
							buffer[16] = '\0';
							l_Rtrim(buffer);
							i2 = l_StrNthChr(dir.datasvcuse, ' ', l_ChrCnt(name, ' ') + 1);
							strncpy(buffer2, dir.datasvcuse, i2);
							buffer2[i2] = '\0';
							if (!stricmp(name, buffer) || !stricmp(name, buffer2))
							{
								fclose(fp1);
						...
						unsigned short *actualdate, unsigned short *actualtime)
{
	...
	sprintf(buffer, "%sF%i.DOP", path1, data.filenbr);
	...
	if (cols[1])
	{
		...
		ti = 0;
	sprintf(buffer, "%sF%i.DAT", path1, data.filenbr);
	if ((fp1 = fopen(buffer, "rb")) == NULL)
...
short      l_CTreadDOP(char *path, short *cols, short *inputfactor, short *dispfactor)
{
	...
	while (fgets(buffer, 80, fp1))
	{
		...
		for (i2 = 0; i2 < 8; i2++)
		{
			if (!strnicmp(fname, fields[i2], strlen(fields[i2])))
			{
				cols[i2] = i1++;
