...
int CAxPattern::Data2Bar()
{
	...
	strcpy(m_bar.description, m_code);
	strcpy(m_bar.symbol, m_code);
...
void CAxPattern::LoadPatternInformation()
{
	...
	fileName.Format("%stab\\patterndesc.txt", m_root);
...
LPCTSTR CAxPattern::GetPatternImage(int pattern)
{
	...
	patternName.Replace(" ", "");
	CString fileName;
	fileName.Format("%sImage\\GTP_%s.bmp", m_root, patternName);
