...
CGrpWnd::CGrpWnd(CWnd *pView, CWnd *pParent, struct _param *pInfo)
{
	...
	m_bSign = true;
	m_bSkipExist = false;
	m_bCtrl = false; //((CMainWnd*)m_pParent)->IsUseCtrl();
	ParseParam((char*)pInfo->options.operator LPCTSTR());	
	m_pApp = (CaxisMThemeApp *)AfxGetApp();
}
...
LONG CGrpWnd::OnUser(WPARAM wParam, LPARAM lParam)
{
	...
	{
	case DLL_OUB:	// set data from server
...
int CGrpWnd::DispatchData(WPARAM wParam, LPARAM lParam)
{
	...
	for ( int ii = 0 ; ii < 2 ; ii++ )
	{
		...
		m_pRgnInfo[m_RgnCnt]->gpRect.SetRectEmpty();
...
void CGrpWnd::ClearGraph(CString sMsg)
{
	m_strMsg = sMsg;
	RemoveComponent();
	Invalidate();
...
void CGrpWnd::DrawGraph(CDC *pDC)
{
	...
	if (m_GrpCnt == 0)
	{
		...
		if (!m_strMsg.IsEmpty())
		{
			CFont	*cFont = m_pApp->GetFont(m_pView, 9);
			CFont	*sFont = pDC->SelectObject(cFont);
			COLORREF	sColor = pDC->SetTextColor(RGB(0, 0, 0));
			pDC->DrawText(m_strMsg, m_GrpRect, DT_VCENTER|DT_SINGLELINE|DT_CENTER);			
...
void CGrpWnd::DrawEmpty(CDC *pDC, bool bInit)
{
	if (bInit)
	{
		...
		fBrush = m_pApp->GetBrush(m_pView, RGB(128,128,128));
...
void CGrpWnd::Resize()
{
	...
	ReleaseDC(pDC);
	int	RgnCnt = (m_RgnCnt > 0)? m_RgnCnt:1;

	double	*pHeight = new double[RgnCnt];
	if (RgnCnt <= 1)
		pHeight[0] = 1.0;
	else
	{
