...
//			break;
	...
	case viewDialog:
		...
		case dlgSFutureCode:			// 20110906 : ADD :
	...
	case viewMain:
		...
		case getPalette:
			...
			break;
		case getHistory:
			//MessageBox((LPCSTR)m_pwndView->SendMessage(WM_USER, MAKEWPARAM(variantDLL, historyCC), lParam), (LPCSTR)lParam);
			return m_pwndView->SendMessage(WM_USER, MAKEWPARAM(variantDLL, historyCC), lParam);

			break;
		case setHistory:
			//MessageBox((LPCSTR)lParam, "setHistory");
...
void CGrpView::SendRequest()
{
	...
	if (m_pwndChart)
	{
		...
		if (memcmp(acInput, "61301", 5)==0)
		{
			CString tmp;
			tmp.Format("%.*s", iDataHLen, pcGDataH);
			tmp.Replace("35500", "65500"); 
			tmp.Replace("34500", "64500"); 
			tmp.Replace("32500", "62500");
			tmp.Replace("\n3", "\n6");
			tmp.Replace("\x7f""3", "\x7f""6");
			memcpy(pcCnvt, (LPCSTR)tmp, iDataHLen);
			pcGDataH = pcCnvt;
		}
	}
