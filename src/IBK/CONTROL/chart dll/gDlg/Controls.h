...
protected:
	...
	afx_msg int OnCreate(LPCREATESTRUCT lpCreateStruct);
	afx_msg void OnLButtonDown(UINT nFlags, CPoint point);
	afx_msg void OnControls(WPARAM wParam, LPARAM lParam);
...
public:
	...
	inline CEdit* GetEditCtrl() { return m_pEdit;}
	inline UINT GetSpinID() { return m_spinID;}
	inline CSpinButtonCtrl* GetSpinCtrl() { return m_pSpinCtrl;}
...
class CAxFontDialog : public CFontDialog
{
	DECLARE_DYNAMIC(CAxFontDialog)

public:
	CAxFontDialog(LPLOGFONT lplfInitial = NULL,
		DWORD dwFlags = CF_EFFECTS | CF_SCREENFONTS,
		CDC* pdcPrinter = NULL,
		CWnd* pParentWnd = NULL);
#ifndef _AFX_NO_RICHEDIT_SUPPORT
	CAxFontDialog(const CHARFORMAT& charformat,
...
class CFontCtrl : public CBaseStatic
{
public:
	CFontCtrl();
	virtual ~CFontCtrl();	
...
public:
	inline UINT GetStaticID() { return m_staticID; }
	inline CStatic* GetStaticCtrl() { return m_pStatic;}
	void SetFontInfo(CString szFaceName, BYTE fstyle, BYTE fweight, BYTE fpoint);
	void SetFontInfo(struct _font*	pFont) { SetFontInfo(pFont->acFName, pFont->btFStyle, pFont->btFWeight, pFont->btFPoint); }
	void GetFontInfo(CString& szFaceName, BYTE& fstyle, BYTE& fweight, BYTE& fpoint);
	void GetFontInfo(struct _font* pFont) 
	{ 
		CString	tmpstr;
		GetFontInfo(tmpstr, pFont->btFStyle, pFont->btFWeight, pFont->btFPoint); 
		memset(pFont->acFName, 0x00, sizeof(pFont->acFName));
		memcpy(pFont->acFName, tmpstr, (tmpstr.GetLength() > sizeof(pFont->acFName) ? sizeof(pFont->acFName) : tmpstr.GetLength()));
	}
...
protected:
	...
	DECLARE_MESSAGE_MAP()
public:
	int     GetData(int nIndex) { return ((CCheckCombo*)m_pCombo)->GetData(nIndex); }
	void    AddItem(CString szContent, int nValue, bool nCheck) { ((CCheckCombo*)m_pCombo)->AddItem(szContent, nValue, nCheck); }
	void	ResetContent() { ((CCheckCombo*)m_pCombo)->ResetContent(); m_pShow->DrawBitmap(false); }
	void    SetCheck(bool bCheck, int nIndex = -1);
	bool    GetCheck(int nIndex = -1) { return ((CCheckCombo*)m_pCombo)->GetCheck(nIndex); }
	int	GetCount() { return m_pCombo->GetCount(); }
	int	GetIndex(int nData) { return ((CCheckCombo*)m_pCombo)->GetIndex(nData); }
