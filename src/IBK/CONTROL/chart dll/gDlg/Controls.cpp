...
void CShowBox::OnControls(WPARAM wParam, LPARAM lParam)
{
...
BOOL CFontCtrl::OnCommand(WPARAM wParam, LPARAM lParam) 
{
	if (LOWORD(wParam) == m_buttonID)
	{
		if (HIWORD(wParam) == BN_CLICKED)
		{
			...
			strcpy(logfont.lfFaceName, m_szFontName);			
...
void CDrawStatic::OnControls(WPARAM wParam, LPARAM lParam)
{
...
void CCheckCBCtrl::OnControls(WPARAM wParam, LPARAM lParam)
{
	if (LOWORD(wParam) == m_ctrlID[1])
	{
		switch (HIWORD(wParam))
