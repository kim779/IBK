...
void CPreviewWnd::drawTICK(CDC* pDC, bool bVol)
{
	...
	for ( int ii = 0 ; ii < nSize ; ii++ )
	{
		...
		rect = CRect(m_arectTick[0].right - size.cx, 0, m_arectTick[0].right,	0);
	...
	if (!bVol)
	{
		...
		for ( ii = 0 ; ii < nSize ; ii++ )
		{
			...
			rect = CRect(m_rectGraph.left + (ii * nWidth), m_arectTick[2].top + 2, 
				m_rectGraph.left + (ii * nWidth) + size.cx, m_arectTick[2].top + 2 + size.cy);
