...
//	Sourcecode welcher TreeListWnd verwendet muss ver?fentlicht werden. 
//	Kommerzielle Nutzer m?sen ihren Code ebenfalls ver?fentlichen, oder 
...
#define 	TVIX_VARBUTTON		0x01						// Schaltf?hen sind nicht fix eingestellt
#define 	TVIX_HASBUTTON		0x02						// Der Eintrag hat eine Schaltf?he
...
struct TreeListData
	...
	int			iStateCheck{};								// Die hState Image-Liste wurde f? die Scheckboxen erzeugt
	...
	int			iIdent{};										// Einr?kung der Kindereint?e
...
int TreeListRegister()
{
	...
	sClass.hInstance	= AfxGetResourceHandle();
	sClass.hIcon		= NULL;
	sClass.hCursor		= LoadCursor(NULL,IDC_ARROW);
	sClass.hbrBackground= NULL;
	sClass.lpszMenuName	= NULL;
	sClass.lpszClassName= _T("CTreeListEx");
	sClass.hIconSm		= NULL;


	if(RegisterClassEx(&sClass)==0)
	{
		const DWORD err = GetLastError();
		TRACE("RegisterClassEx Failed - %X(%d)\n", err, err);
		return 0;
	}

	return (iIsRegistered=TRUE);
}
...
static void CreateStateImageList(TreeListData *pData)
{
	BITMAPINFO	sInfo{};
	//BYTE		aMem[0x1000]{};
	std::unique_ptr < BYTE[] > aMem = std::make_unique<BYTE[]>(0x1000);
	HDC			hDcSrc{};
	HDC			hDc{};
	HBITMAP		hBmp{};
	HBITMAP		hBmpNew{};
	...
	GetDIBits(hDc,hBmp,0,16,aMem.get(),&sInfo,(iBits>8)?DIB_RGB_COLORS:DIB_PAL_COLORS);

	hBmpNew = CreateCompatibleBitmap(hDc,16*3,16);

	SetDIBits(hDc,hBmpNew,0,16,aMem.get(),&sInfo,(iBits>8)?DIB_RGB_COLORS:DIB_PAL_COLORS);
...
{
ExtraItem  *pExtra=nullptr;
BaseItem   *pEntry=nullptr;
HIMAGELIST	hList{};
BITMAPINFO	sInfo{};
std::unique_ptr<BYTE[]> pMem = nullptr;
HDC			hDcSrc{};
HDC			hDc{};
HBITMAP		hBmp{};
HBITMAP		hBmpNew{};
...
LPCSTR		pText=nullptr;
	...
	pMem = std::make_unique<BYTE[]>(iHeigh*(iWidth+4)*4+1024);
	if(!pMem)return NULL;
	...
	GetDIBits(hDc,hBmp,0,0     ,NULL,&sInfo,(iBits>8)?DIB_RGB_COLORS:DIB_PAL_COLORS);
	GetDIBits(hDc,hBmp,0,iHeigh,pMem.get(),&sInfo,(iBits>8)?DIB_RGB_COLORS:DIB_PAL_COLORS);

	hBmpNew = CreateCompatibleBitmap(hDc,iWidth,iHeigh);

	SetDIBits(hDc,hBmpNew,0,iHeigh,pMem.get(),&sInfo,(iBits>8)?DIB_RGB_COLORS:DIB_PAL_COLORS);
...
TEXTMETRIC	sMetrics{};
BaseItem   *pEntry=nullptr;
BaseItem  **pList=nullptr;
ExtraItem  *pExtra=nullptr;
ExtraItem **pItems=nullptr;
unsigned	uSub=0;
...
unsigned	uTemp=0;
unsigned	uStart=0;
unsigned   *pLines=nullptr;
BaseItem  **pItems=nullptr;
BaseItem   *pEntry=nullptr;
BaseItem   *pTemp=nullptr;
RECT		sRect{};
...
//	pData		: Zeiger auf die Fensterdaten
//	Ergibt die Breite ab der die Spaltenver?dert wurden oder 0x10000
...
//						TVC_BYMOUSE
//						TVC_UNKNOWN
//	Ergibt		2 wenn der Eintrag gew?lt und umgeklapt wurden
//				1 wenn der Eintrag gew?lt
...
char*		pTemp = nullptr;
LPCTSTR	    pText = nullptr;
LPCTSTR	    pTextTemp = nullptr;
ExtraItem** pExOld[MAX_COLUMNS]{};
ExtraItem** pExNew[MAX_COLUMNS]{};
BaseItem   *pNew = nullptr;
BaseItem  **pOld = nullptr;
BaseItem  **pItems = nullptr;
BaseItem   *pEntry = nullptr;
BaseItem   *pParent = nullptr;
unsigned   *pPosNew = nullptr;
unsigned   *pPosOld = nullptr;
unsigned   *pFirst = nullptr;
unsigned   *pLast = nullptr;
unsigned	uParent=0;
unsigned	uSize=0;
unsigned	uBits=0;
unsigned	uItem=0;
...
//******************** Den neuen Eintrag erzeugen *****************************
	...
	const int len = sizeof(BaseItem) + pData->uUserDataSize;
...
//******************** Eintrage einf?en **************************************
	...
	case U(TVI_SORT):										// Alphapetisch einf?en
			...
			if(uItem==pFirst[0])							// Am Anfang einf?en
				{
				pFirst[0]=uPos;
				}
 	...
 	case U(TVI_ROW):										// In einer genauen Reihe einf?en
			...
			for(;uItem;uItem=pItems[uItem]->uNextItem)		// Suche Einf?eposition
				{
...
unsigned	uPos=0;
	...
	if(pData->uColumnCount>0)								// Liste mit Extraeintr?en l?chen
		...
		if(pList)
			...
			if (pData->uColumnCount > -1 && pData->uColumnCount < MAX_COLUMNS-1)
	...
	if (pData->uColumnCount > -1 && pData->uColumnCount < MAX_COLUMNS )
	...
	if(!pData->uColumnCount)								// Den Header l?chen
	   ...
	   iXoff=UpdateCoumns(pData);							// Hat sich die Spaltenbreiten ver?dert
	if(iXoff<0x10000)
		{
		sRect.left  = iXoff;
		sRect.left -= pData->uScrollX;
...
unsigned	uSub=0;
	...
	if(iXpos<pData->iIdent)									// Auf einger?ktem Bereich
		{
...
//*
//*****************************************************************************
//	Sortiert die Kindereintr?e eines Eintrages mittele einer Funktion
//	pData		: Zeiger auf die Fensterdaten
...
//******************** Erzeugre Eintragsliste *********************************
	unsigned   *pItemList = nullptr;
	unsigned   *pItemNew = nullptr;
...
#define XCHANGE_MEM(a,b)	uTemp=pItemList[a];pItemList[a]=pItemList[b];pItemList[b]=uTemp;
	...
	LPARAM			lParamTemp{};
	LPARAM			lParamSort=0;
	int				iLower=0,iUpper=0,iMiddle=0,iCmp=0;
...
static HWND TreeListEditLabel(TreeListData *pData,unsigned uItem,unsigned uSub)
{
	...
	if(pData->hEdit && (uBits&3)!=pData->uEditMode)			// Editfenster bei Typ?derung l?chen
		{
	...
	if(uSub==0)												// Haupteintrag bearbeiten
	{
		...
		if(pEntry->bCallback&TVIF_TEXT)
			{
			CallbackEntry(pData,pEntry,uItem,TVIF_TEXT,iTemp,uSize,(LPCTSTR &)pText);
			hDc=GetDC    (pData->hWnd);
			SelectObject (hDc,hFont);
			DrawText     (hDc,pText,0,&sRect,DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_NOPREFIX|DT_CALCRECT);
			ReleaseDC    (pData->hWnd,hDc);
		...
		if(!pExtra)
			...
			if(pExtra->bCallback&TVIF_TEXT)
				{
				CallbackExtra(pData,pEntry,pExtra,uItem,uSub,TVIF_TEXT,iTemp,uSize,(LPCTSTR &)pText);
				hDc=GetDC    (pData->hWnd);
				SelectObject (hDc,hFont);
				DrawText     (hDc,pText,0,&sRect,DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_NOPREFIX|DT_CALCRECT);
				ReleaseDC    (pData->hWnd,hDc);
...
//	TreeListData* pData = nullptr;
	...
	case WM_CREATE:											// Das Fenster erzeugen
			...
			//pData = std::make_shared<TreeListData>().get();
...
//			pData->pTreeItems = (BaseItem **)std::make_shared<BaseItem*[]>(1).get();//  new BaseItem* [1];
			if(!pData->pTreeItems)
				{
				delete pData;
				return -1;
...
//			pData->pItemPos = std::make_unique<unsigned[]>(1);
			if(!pData->pItemPos)
				{
...
ExtraItem	   *pExtra = nullptr;
LPCTSTR			pText{};
unsigned		uTextSize=0;
unsigned		uRgnCount=0;
unsigned		uStyleEx=0;
unsigned		uStyle=0;
unsigned		uExtra=0;
unsigned		uMark=0;
