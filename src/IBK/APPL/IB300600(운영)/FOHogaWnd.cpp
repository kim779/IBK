...
//	m_pBGrid = new CGridCtrl();
	m_pBGrid = new BottomGrid(pParent);
...
void CFOHogaWnd::SetPprcData(int idata)
{
	m_pGrid->SetPprcData(idata);
}
...
// 	m_pBGrid->Create(rc, this, IDC_GRID_BOTTOM); //test
	m_pBGrid->Create(rc, this, IDC_GRID_BOTTOM, WS_CHILD|WS_BORDER|WS_VISIBLE);
	SetBottomGrid();
...
void CFOHogaWnd::CalcSize()
{
	...
	//2017.01.16 KSJ 사이즈 조절 + crc.Width()/2 주석처리
	m_pMarkMado->MoveWindow(crc.left /*+ crc.Width()/2*/, pos_y, crc2.right-(crc.left /*+ crc.Width()/2*/)+2, GRID_CELL_HEIGHT, FALSE);

	int nMarkWidth;

	//2017.01.16 KSJ 사이즈 조절 + crc.Width()/2 주석처리
	nMarkWidth = crc2.right-(crc.left /*+ crc.Width()/2*/)+2;
	...
	m_pMarkMasu->MoveWindow(crc.left, pos_y, nMarkWidth, GRID_CELL_HEIGHT, FALSE);
...
//slog.Format("[KSJ] m_pRsvdData1 : odgb[%s] oqty[%d] mult_cnt[%d] mult_qty[%d] tot_dcnt[%d] tot_dwqty[%d] tot_scnt[%d] tot_swqty[%d]\n", st->odgb ==1 ? "매도":"매수", st->oqty, mult_cnt, mult_qty, tot_dcnt, tot_dwqty, tot_scnt, tot_swqty);
...
//slog.Format("[KSJ] m_pRsvdData2 : st odgb[%d] cprc[%d] prd odgb[%d] cprc[%d]\n", st->odgb, st->cprc, prd->odgb, prd->cprc);
//OutputDebugString(slog);

				if(st->oprc == MARKET_JPRC)
				{
					AddArrow( CCellID(st->row, st->col), 9999 );
				}
				else
				{
		...
		{
//slog.Format("[KSJ] DELARROW : row[%d] col[%d] ptic[%d]\n",prd->row,prd->col,prd->ptic);
//OutputDebugString(slog);
...
/*
CString slog;
slog.Format("[3006] onnotify IDC_GRID_BOTTOM [%d]   row=[%d] col=[%d]\n", pi->hdr.code, pi->item.row, pi->item.col);
OutputDebugString(slog);
*/

	}
	else if (wParam==IDC_GRID_HOGA)
	{
		*pResult = 1;
...
CString slog;
slog.Format("[3006] onnotify [%d]   row=[%d] col=[%d]\n", pi->hdr.code, pi->item.row, pi->item.col);
OutputDebugString(slog);
		...
		else if (pi->hdr.code == GVN_SELCHANGED)
		{
			...
			bOneClick = m_bOneClickOrder && (m_pGrid->GetFocusCell()==m_pGrid->GetLeftClickDownCell()) && (!m_bMouseOut) && (!m_bKeyUPDOWN);
		...
		if ((view->hdr.code == NM_DBLCLK) && (view->iColumn<=IDX_ASK_JUMUN || view->iColumn>=IDX_BID_JUMUN))
	 	...
	 	if ((view->hdr.code==NM_DBLCLK && !m_bOneClickOrder) || 
			...
			(view->hdr.code==NM_DBLCLK && m_bDragDropOrder) ||
			(view->hdr.code==NM_DBLCLK && m_bDragDropJung))  //잔고 클릭드래그드랍해서 주문 했을경우  
		{
...
OutputDebugString(slog);
			...
			if (!bJumn && m_bExtOdrColumn && !bDragOrder && !bDragJung)
			{
			...
			if (view->iRow>0 && bJumn)
			{
				...
				if(bDragOrder && (view->iColumn==IDX_ASK_JUMUN || view->iColumn==IDX_BID_JUMUN)) //드래그 주문인경우 매수주문이나 매도주문컬럼에서만
						GetParent()->SendMessage(WM_APP_SIG, WP_DRAG_ORDER, (LPARAM)&odr);
				else if(bDragJung && (view->iColumn==IDX_ASK_JUMUN || view->iColumn==IDX_BID_JUMUN)) //드래그 정정주문인경우 매수주문이나 매도주문컬럼에서만
				{
					ModifyData md;
					
					if(m_iDragDropJungMS == IDX_ASK_JUMUN)
						md.odgb = 1;
					else if(m_iDragDropJungMS == IDX_BID_JUMUN)
						md.odgb = 2;
					
					if (md.odgb>0)
					{
						//	md.fprc = m_pGrid->GetItemData(id.row, IDX_HOGA);
						md.tprc = m_pGrid->GetItemData(id.row, IDX_HOGA);
						//	md.rtic = m_DragStartId.row - id.row;   WP_MODIFY_ORDER
						GetParent()->SendMessage(WM_APP_SIG, WP_MODIFY_DRAGORDER, (LPARAM)&md);
					}
					m_iDragDropJungMS = 0;
				}
				else
...
OutputDebugString(slog);
			...
			else if (view->iRow>0 && bRsvd && !bDragOrder && !bDragJung)
			{
				ReserveData rd;
				rd.row = view->iRow;
				rd.col = view->iColumn;
...
void CFOHogaWnd::AddArrow( CCellID id, int tick )
{
	...
	if (id.col==IDX_ASK_RSVD)
	{
		...
		ar.m_st.y = crc.top + m_iRowHeight/2;   //test height
		ar.m_ed.x = crc.right + (LONG)(m_pGrid->GetColumnWidth(IDX_ASK_JUMUN)*0.8);
		ar.m_ed.y = ar.m_st.y - (tick * m_iRowHeight);  //test height
	...
	else if (id.col==IDX_BID_RSVD)
	{
		...
		ar.m_st.y = crc.top + m_iRowHeight/2;  //test height
		ar.m_ed.x = crc.left - (LONG)(m_pGrid->GetColumnWidth(IDX_BID_JUMUN)*0.8);
		ar.m_ed.y = ar.m_st.y - (tick * m_iRowHeight);  //test height
		ar.m_eid.row = id.row - tick;
		ar.m_eid.col = id.col - 1;
	}	
...
void CFOHogaWnd::SetAverage( CCellID id, int tick )
{
	CArrow ar;
	CRect crc;
	...
	if (!m_pGrid->GetAbsCellRect(CCellID(id.row,IDX_HOGA), crc)) return;

	CRect rcJan;

	m_pGrid->GetAbsCellRect(CCellID(id.row,IDX_HOGA+1), rcJan);
	
	if (id.col==IDX_ASK_RSVD)
	{
		ar.m_st.x = rcJan.left - 3;
		ar.m_st.y = crc.top + m_iRowHeight/2;  //test height
		ar.m_ed.x = crc.right + (LONG)(m_pGrid->GetColumnWidth(IDX_HOGA)*0.8);
		ar.m_ed.y = ar.m_st.y - (tick * m_iRowHeight);  //test height
		ar.m_eid.row = id.row - tick;
		ar.m_eid.col = id.col + 1;
	...
	else if (id.col==IDX_BID_RSVD)
	{
		ar.m_st.x = crc.left;
		ar.m_st.y = crc.top + m_iRowHeight/2;  //test height
		ar.m_ed.x = crc.left - (LONG)(m_pGrid->GetColumnWidth(IDX_HOGA)*0.8);
		ar.m_ed.y = ar.m_st.y - (tick * m_iRowHeight);  //test height
		ar.m_eid.row = id.row - tick;
		ar.m_eid.col = id.col - 1;
...
// 	m_pGrid->SetAverage(ar);
// #endif
}

void CFOHogaWnd::RefreshAvearage()
{
	CArrow ar;
	CRect crc;

	ar.m_sid = m_pGrid->m_arrAvr.m_sid;
	
	if (!m_pGrid->GetAbsCellRect(CCellID(m_pGrid->m_arrAvr.m_sid.row,IDX_HOGA), crc)) return;
	
	CRect rcJan;
	
	m_pGrid->GetAbsCellRect(CCellID(ar.m_sid.row,IDX_HOGA+1), rcJan);
	
	if (ar.m_sid.col==IDX_ASK_RSVD)
	{
		ar.m_st.x = rcJan.left - 3;
		ar.m_st.y = crc.top + m_iRowHeight/2;  //test height
		ar.m_ed.x = crc.right + (LONG)(m_pGrid->GetColumnWidth(IDX_HOGA)*0.8);
		ar.m_ed.y = ar.m_st.y;
		ar.m_eid.row = ar.m_sid.row;
		ar.m_eid.col = ar.m_sid.col + 1;
		
		ar.m_bmarket = TRUE;
	}
	else if (ar.m_sid.col==IDX_BID_RSVD)
	{
		ar.m_st.x = crc.left;
		ar.m_st.y = crc.top + m_iRowHeight/2;  //test height
		ar.m_ed.x = crc.left - (LONG)(m_pGrid->GetColumnWidth(IDX_HOGA)*0.8);
...
{
	//2017.01.16 KSJ 시장가 더블클릭이면 여기 타면 안됨
...
{
	//2017.01.16 KSJ 시장가 더블클릭이 아니면 여기 타면 안됨
	if(!m_bSiJangDBClick)	return;
...
OutputDebugString(slog);
	...
	for(int ii = 0 ; ii < irow; ii++)
		m_pGrid->SetRowHeight(ii, m_iRowHeight);
	m_pGrid->DelAllArrow();
	ReDrawReserveArrow();
	m_pGrid->Invalidate();
	m_pGrid->SetCurrToCenter();
...
void CFOHogaWnd::ReDrawReserveArrow()
{
	AccountMap::iterator ast = m_pAccounts->begin(), aed = m_pAccounts->end();
	int oqty=0, ocnt=0;
	int mult_cnt = m_pAccounts->size();
