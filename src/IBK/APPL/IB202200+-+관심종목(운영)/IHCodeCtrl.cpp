...
//		return true;
	...
	for (int ii = 0; ii < codeN; ii++)
	{
		...
		int	res{};
		CString tmp = CString(OJCode.price, OPriceLen);
		oCode.Price.Format("%3.1f", atof(tmp)/ 100.0);
...
// updateX_20060209
	CString	Acode, Jcode, Qcode;
	Acode = "A" + sCode; Jcode = "J" + sCode; Qcode = "Q" + sCode;	//2014.11.13 KSJ ETN 추가
	...
	for (int ii = 0; ii < m_pJCode.GetSize(); ii++)
	{
		...
		if (!Acode.Compare(jCode.Code) || !Jcode.Compare(jCode.Code) || !Qcode.Compare(jCode.Code))
			return true;
	}
