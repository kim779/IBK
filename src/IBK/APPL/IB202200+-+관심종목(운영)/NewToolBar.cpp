...
{	
  ...
  case CNewMenu::STYLE_XP_NOBORDER:
    if(pInfo->uItemState&CDIS_HOT && !(pInfo->uItemState&CDIS_SELECTED))
    {
      ...
      pDC->DrawState(ptImage, imgSize, hDrawIcon2, DSS_MONO,(HBRUSH)nullptr);
  ...
  default:
    ...
    if(m_ImageList.GetSafeHandle())
    {
      ...
      if(::ImageList_GetIconSize(m_ImageList,&cx,&cy))
      {
        ...
        {
          pDC->DrawState(ptImage, size, hDrawIcon, DSS_DISABLED, (HBRUSH)nullptr);
        }
        else
        {
          pDC->DrawState(ptImage, size, hDrawIcon, DSS_NORMAL, (HBRUSH)nullptr);
        }
