...
////////////////////////////////////////////////
	...
	m_btTEXT[0] = std::make_unique<CfxImgButton>();
	...
	m_btTEXT[1] = std::make_unique<CfxImgButton>();
...
// 	m_btHAWKEYE.ShowWindow(SW_HIDE);
	...
	// m_btBASKET = std::make_unique<CfxImgButton>();
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		rect = MoveButton(rect, CSize(SIZE_BUTTONS, SIZE_BUTTONS), CSize(0 , SIZE_GAPS + 1), m_btCHART.get());
...
//		rect = GetNextRect(rcTemp, CSize(SIZE_COMBO, SIZE_BUTTONS), CSize(0, SIZE_GAPS));
		...
		*/
		rect = MoveButton(rect, CSize(85, SIZE_BUTTONS), CSize(SIZE_GAPS , 0), m_btField.get());
		...
		rect = MoveButton(rect, CSize(38, SIZE_BUTTONS), CSize(SIZE_GAPS , 0), m_btTEXT[0].get());
		rect = MoveButton(rect, CSize(36, SIZE_BUTTONS), CSize(SIZE_GAPS , 0), m_btTEXT[1].get());
		...
		m_arCONTROL.Add((CWnd*)m_chkAUTO.get());

		// plus minus
		rect = MoveButton(rect, CSize(SIZE_BUTTONS, SIZE_BUTTONS), CSize(SIZE_GAPS , 0), m_btMINUS.get());
		rect = MoveButton(rect, CSize(SIZE_BUTTONS, SIZE_BUTTONS), CSize(SIZE_GAPS , 0), m_btPLUS.get());
...
//		rect = MoveButton(rect, CSize(SIZE_BUTTONS, SIZE_BUTTONS), CSize(SIZE_GAPS , 0), &m_btHAWKEYE);
		...
		m_arCONTROL.Add((CWnd*)m_chkEXPAND.get());
...
//		rect.top = rcTemp.top;
		rcTemp = rect = MoveButton(rect, CSize(SIZE_BUTTONS, SIZE_BUTTONS), CSize(SIZE_GAPS , 0), m_btAddSelect.get());
		rect = MoveButton(rect, CSize(SIZE_BUTTONS, SIZE_BUTTONS), CSize(SIZE_GAPS , 0), m_btAddVisible.get());
//		rect = MoveButton(rect, CSize(SIZE_BUTTONS, SIZE_BUTTONS), CSize(SIZE_GAPS , 0), &m_btAddFull);
		m_btAddFull->ShowWindow(SW_HIDE);

		// 도구
		rcTemp = rect = MoveButton(rect, CSize(36, SIZE_BUTTONS), CSize(SIZE_GAPS , 0), m_btTool.get());
		...
		// 돌려보기
		rect = MoveButton(rect, CSize(SIZE_BUTTONS, SIZE_BUTTONS), CSize(SIZE_GAPS , 0), m_btTURN.get());
