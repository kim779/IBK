...
//		CopyMemory(&m_logfont, logfont, sizeof(LOGFONT));
	...
	_font.CreatePointFont(9, "굴림체");
	_pen.CreatePen(PS_SOLID, 1, RGB(128, 128, 128) );
	_penUP.CreatePen(PS_SOLID, 1, RGB(255, 0, 0)   );
	_penDOWN.CreatePen(PS_SOLID, 1, RGB(0, 0, 255) );
	_pena.CreatePen(PS_SOLID, 2, RGB(0, 0, 255));
	_brushUP.CreateSolidBrush(RGB(255, 0, 0));
	_brushDOWN.CreateSolidBrush(RGB(0, 0, 255));
	_brushSISE.CreateSolidBrush(RGB(255, 255, 128));
	_brushNEWS.CreateSolidBrush(RGB(128, 128, 255));
	_penSig.CreatePen(PS_SOLID, 1, RGB(200, 200, 200));
...
CintGrid::~CintGrid()
{
	...
	_pen.DeleteObject();
	_penUP.DeleteObject();
	_penDOWN.DeleteObject();
	_brushUP.DeleteObject();
	_brushDOWN.DeleteObject();
	...
	if(m_pBmpInfo24)
		m_pBmpInfo24->DeleteObject();
	if(m_pTBmp)
		m_pTBmp->DeleteObject();
	if(m_pBBmp)
		m_pBBmp->DeleteObject();
...
CWnd* CintGrid::Create(const RECT& rect, CWnd* parent, UINT nID, int NOscroll, int dragdrop, DWORD dwStyle)
{
	...
	//2017.01.25 KSJ GetBuffer(0) 삭제
	HBITMAP hBitmap = (HBITMAP)::LoadImage(nullptr, fileIMG,
...
// 
// 	if(nCol == colCURR && (bExpect || atoi(str)) && string != "예상가" && strCode.GetLength() == 6 /*&& strCode.GetLength() == 6*/)	//2011.12.22 KSJ 코드길이가 6일때만 예상가 그리도록 2013.08.05 KSJ 선옵, 지수, 주식선물일때도 예상가 표시하도록 요청받음.
// 	{		
// 		CDC buffDC;
...
BOOL CintGrid::SetItem(const GVITEM* gvitem)
{
	...
	if (gvitem->mask & GVMK_FONT)    pCell->font  = gvitem->font;		//memcpy(&(pCell->font), &(gvitem->font), sizeof(LOGFONT));
...
BOOL CintGrid::GetItem(GVITEM* gvitem)
{
	...
	if (gvitem->mask & GVMK_FONT)    gvitem->font  = pCell->font;	//memcpy(&(gvitem->font), &(pCell->font), sizeof(LOGFONT));
...
void CintGrid::OnDestroy() 
{
	...
	if (nullptr != m_pBmpInfo24)
	{
		HBITMAP hBitmap = (HBITMAP)m_pBmpInfo24->Detach();
	...
	if(nullptr != m_pTBmp)
	{
		HBITMAP hBitmap = (HBITMAP)m_pTBmp->Detach();
	...
	if(nullptr != m_pBBmp)
	{
		HBITMAP hBitmap = (HBITMAP)m_pBBmp->Detach();
...
//AxStd::_Msg();
	...
	if (curr > open)
	{
		pDC->SelectObject(&_penUP);
		aval = open;
		bval = curr;
	}
	else if (curr < open)
	{
		pDC->SelectObject(&_penDOWN);
		pDC->SelectObject(&_brushDOWN);
		aval = curr;
		bval = open;
	}
	else
...
void CintGrid::testSaveFile(CString datB)
{
	return;
	CString strPath(_T("")), titletime;
	strPath.Format("%s\\2022Log.dat", m_root);
	...
	file.Open(strPath, CFile::modeNoTruncate | CFile::modeCreate | CFile::modeWrite | CFile::typeText|CFile::shareDenyNone);
	file.SeekToEnd(); 	
	
	titletime.Format("[%s][202200][%s]\n", strCurTime, datB);
...
void CintGrid::DrawMarginRatio(CDC* pDC, CRect rect, int nRow)
{
	...
	//2017.01.25 KSJ GetBuffer(0) 삭제
	hBitmap = (HBITMAP)::LoadImage(nullptr, strImgPath,
		IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION); 

	m_pTBmp = CBitmap::FromHandle(hBitmap);

	pOldBmp = memDC.SelectObject(m_pTBmp);
	...
	m_pBBmp = CBitmap::FromHandle(hBitmap);
	
	pOldBmp = memDC.SelectObject(m_pBBmp);
...
{
	//2017.01.25 KSJ GetBuffer(0) 삭제
	HBITMAP hBitmap = (HBITMAP)::LoadImage(nullptr, strFile,
					  IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION); 
	...
	case 24:
		{
			...
			{	
				...
				m_pBmpInfo24 = CBitmap::FromHandle(hBitmap);
			}
		}
		break;
