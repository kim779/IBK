...
void CExSpin::OnSize(UINT nType, int cx, int cy) 
{
	...
	const int swid = GetSystemMetrics(SM_CYHSCROLL);
	const int iWidth = cx - swid;
...
void CExSpin::createControls()
{
	...
	if (!m_pEdit) 
	{
		m_pEdit = std::make_unique<CSpinEdit>(this, m_attr);
	...
	if (!m_pSpin) 
	{
		m_pSpin = std::make_unique<CSpinCtrl>(this, m_attr, m_spType);
