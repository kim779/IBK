...
void CAxMapForm::CreateMap(CWnd *pParent, CSize szForm)
{
	...
	Create(nullptr, nullptr, WS_CHILD | WS_VISIBLE/* | WS_CLIPCHILDREN*/, CRect(0, 0, szForm.cx, szForm.cy), &wnd, 100, nullptr);
	wnd.Detach();
...
CWnd* CAxMapForm::CreateMapObject(CString sMapName, int &nObjKey, CPoint point)
{
	if (!sMapName.IsEmpty())
	{
		...
		if (nType != vtypeERR)
		{
			auto pObjWnd = std::make_unique<CWnd>();
			pObjWnd->Create(nullptr, nullptr, WS_CHILD, CRect(0, 0, LOWORD(nSzMap), HIWORD(nSzMap)), this, 0);
			nObjKey = m_pParent->SendMessage(WM_USER, MAKEWPARAM(attachMAP, nType), (long)pObjWnd.get());
			sStr = sMapName;
			m_pParent->SendMessage(WM_USER, MAKEWPARAM(loadMAP, nObjKey), (long)sStr.operator LPCTSTR());
			pObjWnd->SetWindowPos(nullptr, point.x, point.y, 0, 0, SWP_NOZORDER|SWP_NOSIZE);
		}
	}
	return nullptr;
}
