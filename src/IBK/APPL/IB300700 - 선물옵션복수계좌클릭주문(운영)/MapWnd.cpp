...
int CMapWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_pGroup->Create(WS_CHILD|WS_VISIBLE|WS_VSCROLL|CBS_DROPDOWNLIST|CBS_AUTOHSCROLL, rc, this, IDC_CB_GROUP);	//2016.01.08 KSJ 스크롤바 생기게 변경
...
// 	}
	...
	// 계좌 윈도우
	{
		...
		if (m_AcntMode==AM_GROUP)
		{
			...
			m_pGroup->MoveWindow(pos_x + 40, pos_y, 140, 500, FALSE);	//2016.01.08 KSJ 콤보박스 사이즈 변경
		...
		m_pOptionEx->MoveWindow(pos_x, pos_y, 38, btn_h, FALSE);pos_x += 44 + PADDING_VALUE/2;
		m_pOptionEx->ChangeFont(9, _T("굴림체") );

		m_pStCodeNm->MoveWindow(pos_x, pos_y, 150, btn_h, FALSE);
...
// 	m_SiseData.shga = fabs(Str2Double(mod->shga, sizeof(mod->shga))*100) + DOUBLE_PREFIX;
// 	m_SiseData.gjga = fabs(Str2Double(mod->gjga, sizeof(mod->gjga))*100) + DOUBLE_PREFIX;
// 	m_SiseData.siga = fabs(Str2Double(mod->siga, sizeof(mod->siga))*100) + DOUBLE_PREFIX;
// 	m_SiseData.koga = fabs(Str2Double(mod->koga, sizeof(mod->koga))*100) + DOUBLE_PREFIX;
...
// 	m_SiseData.curr = fabs(Str2Double(mod->curr, sizeof(mod->curr))*100) + DOUBLE_PREFIX;
	...
	{
		//      m_SiseData.hoga[n].msga = Str2Double(mod->hoga[n].msga, sizeof(mod->hoga[n].msga))*100 + DOUBLE_PREFIX;
		//      m_SiseData.hoga[n].mdga = Str2Double(mod->hoga[n].mdga, sizeof(mod->hoga[n].mdga))*100 + DOUBLE_PREFIX;
...
//2013.10.11 KSJ 암호화비번 추가
LPCSTR CMapWnd::GetEnPassword( LPCSTR acno )
{
	AccountMap::iterator pos = m_AcntData.find(acno);
	if (pos==m_AcntData.end()) return "";
	else return (LPCSTR)pos->second.epswd;
}
...
void CMapWnd::ChangeCode( LPCSTR code , BOOL bTrigger /*=TRUE*/)
{
	...
	*/
	...
	if (_code.Mid(1,2) != "05")
	{
		m_pControl->SetFOType(TRUE);
	}
	else
	{
		//_name = "M " + _name;
		m_pControl->SetFOType(FALSE);
	}
...
// 	}
	...
	// 데이터 세팅
#ifdef _DEF_IB300600_
	CopyMemory(ledger->hsiz, "44", 2);
	CopyMemory(ledger->epwd, GetEnPassword(acno), 44);
#else
	CopyMemory(ledger->hsiz, "44", 2);
	FillMemory(ledger->epwd, sizeof(ledger->epwd), ' ');
	CopyMemory(mid->gubn, "^A", 2);
#endif
...
#else
	CopyMemory(mid->hash, GetEnPassword(acno), 44);
#endif
...
void CMapWnd::merge_order( vector<char> &buff, vector< vector<char> > &orders )
{
	...
	// 레코드갯수 조정
	sprintf(tmp, "%s", "^A");
	buff.insert(buff.end(), tmp, tmp+2);
...
//2013.10.11 KSJ 암호화비번 추가
void CMapWnd::AddAccount(LPCSTR acno, LPCSTR pswd, LPCSTR acnm, int mult, LPCSTR epswd)
{
	AccountMap::iterator pos = m_AcntData.find(acno);
	if (pos==m_AcntData.end()) 
	{
		...
		account.mult = mult;
		account.pswd = pswd;
		account.epswd = epswd;	//2013.10.11 KSJ 암호화비번 추가
...
void CMapWnd::req_SDLDQ003_grp(BOOL bNext /*=FALSE*/, LPCSTR nkey /*=NULL*/)
{
	...
	CopyMemory(ledger->rcnt, "0040", 4);		//2016.08.30 KSJ  20 -> 40
...
// 			OutputDebugString("[KSJ] group[" + gseq + ". " + gnam + "]");
		}
		if (nrec) 
		{
			m_LastGroup = "";
...
// 			FillMemory((char*) &item, sizeof(_PIBFUTP1_item), ' ');
			...
			AddAccount(acno, "HEAD", acnm, atoi(mult), pswd);	//2013.10.11 KSJ 암호화비번 추가
...
void CMapWnd::reg_order( struct _ledgerH *ledger, vector< vector<char> > &orders)
{
	...
	//2013.10.14 KSJ NULL값이 들어가지 않도록 수정
	CString strRegID;
	strRegID.Format("%06d", regid++);

	CopyMemory(&ledger->fil2[ORDER_INFO_IDX], strRegID, strRegID.GetLength());
