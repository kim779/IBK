...
//TRACE("그룹 sAccn[%s] keyS[%s] sData[%s]\n", sAccn, keyS, sData);
	...
	CAccn *pAccn = NULL;
	//std::unique_ptr<CAccn> pAccn = nullptr;
...
// 		CString strTemp;
// 		strTemp.Format("OnReceiveRemainData sAccn[%s] keyS[%s] creS[%s] sData[%s]", sAccn, keyS, creS, sData);
...
//	std::unique_ptr<CAccn> pAccn = nullptr;
	CString sData;
	int skey=-1;
	CString AccHandleKey, sHandle; sHandle.Format("%d",(long)sHwnd);
	if (m_AccnKey.Lookup(sHandle,AccHandleKey))
	{
		
		CString strTemp;
		strTemp.Format("[KSJ] Exist sHandle[%s] AccHandleKey[%s]", sHandle, AccHandleKey);
...
//			pAccn = new CAccn;
			...
			if(bFuture)			
				pAccn->m_bReceive = false;	//2015.03.04 KSJ 리시브 받지 않으면 잔고화면에 추가하지않는다.
			
			const int kNum = GetNewAccHandleKey();
			AccHandleKey.Format("%d",kNum);

// 			strTemp.Format("[KSJ] Next1 sHandle[%s] AccHandleKey[%s]", sHandle, AccHandleKey);
// 			OutputDebugString(strTemp);
...
//		pAccn = std::make_unique<CAccn>();
		...
		CString strTemp;
		strTemp.Format("[KSJ] Next2 sHandle[%s] AccHandleKey[%s]", sHandle, AccHandleKey);
		OutputDebugString(strTemp);
...
// 		OutputDebugString("[KSJ] ledger+mid[" + sData + "]");
...
// 			}
			...
			CString strTemp;
			strTemp.Format("MAIN parsingRemainData sCode[%s] trkey[%d]", sCode, trkey);
			pRemain->testSaveFile(strTemp);
		}
		else
		{
	...
	for (AccnPos = m_AccnMap.GetStartPosition(); AccnPos != NULL;)
	{
		...
		if (pAccn->m_bFuture)
		{
			...
			if (pAccn->m_CodeMap.Lookup(keyS, (void*&)pRemain))
			{
				...
				CString strTemp;
				strTemp.Format("[KSJ][parsingRTS] nCount[%d] sAccn[%s] pAccn->m_accn[%s] keyS[%s] sCode[%s] data[%s]", nCount++, sAccn, pAccn->m_accn, keyS, sCode, pRemain->TotalData());
				pRemain->testSaveFile(strTemp);
				//OutputDebugString(strTemp);
			}
		}
...
/*
	...
	else*/ if (!symMap.Lookup("023", sCurr))
		return;
...
{
// 	CString strTemp;
// 	strTemp.Format(("[KSJ] sTR[%s] sData[%s] nKey[%d] nStat[%d] acnt[%s]"), sTR, sData, nKey, nStat, Acnt);
// 	OutputDebugString(strTemp);
...
BOOL CMapWnd::LoadCode()
{
	...
	if (!file.Open(string, CFile::modeRead|CFile::typeBinary|CFile::shareDenyRead))
		return FALSE;
